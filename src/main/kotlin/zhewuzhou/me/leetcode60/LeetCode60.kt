...
fun getPermutation(n: Int, k: Int): String {
    ...
    backtrace(res, mutableListOf(), source, k, false)
    return res[res.lastIndex].joinToString("") { it.toString() }
}

private fun backtrace(res: MutableList<List<Int>>, comb: MutableList<Int>, source: List<Int>, target: Int, found: Boolean) {
    var f = found
    if (res.size == target) {
        f = true
    }
            ...
            if (!f) {
                for (i in source.indices) {
                    if (!comb.contains(source[i])) {
                        comb.add(source[i])
                        backtrace(res, comb, source, target, f)
